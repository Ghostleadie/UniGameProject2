...
public:
	...
	SceneComponent(ModelManager* mMan)
	{
		...
		loadSceneJSON(levelVec[0], *ModelMan);
	}
...
private:
	...
	void loadDynamicObjects(Json::Value root, Json::Reader reader, ModelManager& modelMan) {
		...
		for (int i = 0; i < dynamicGameObjects.size(); i++)
		{
			...
			const Json::Value ObjSize = dynamicGameObjects[i]["size"];
			...
			glm::vec3 size(x, y, z);
			float rad = max(max(x, y), z);


			v_dynamicLevelObjects[i].addComponent(new PhysicsComponent(pos, rad, size));
	...
	void loadStaticObjects(Json::Value root, Json::Reader reader, ModelManager& modelMan) {
		...
		for (int i = 0; i < staticGameObjects.size(); i++)
		{
			...
			glm::vec3 scale(x, y, z);
			const Json::Value ObjSize = staticGameObjects[i]["size"];
			...
			v_staticLevelObjects[i].addComponent(new PhysicsComponent(pos, rad, size));
